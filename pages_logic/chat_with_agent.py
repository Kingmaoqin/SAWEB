...
def show():
    ...
    if not st.session_state.chat_greeted:
        if has_data:
            greet = ("Hello! Your dataset is loaded. You can preview TEXGI or train MySA from the right, "
                    "or type 'preview mysa' / 'train mysa lambda=0.1'.")
        else:
            greet = ("Hello! Please upload a CSV on the right first. Then you can preview TEXGI or train MySA "
                    "with commands like 'preview mysa' / 'train mysa lambda=0.1'.")
    ...
    with left:
        ...
        with st.container():
            ...
            if cols_qa[0].button("Preview FI (MySA)", use_container_width=True, disabled=not has_data):
                t = tcol or "duration"; e = ecol or "event"
                _run_direct("TEXGISA", t, e, epochs=80, preview=True)

            if cols_qa[1].button("Train MySA with priors", use_container_width=True, disabled=not has_data):
                t = tcol or "duration"; e = ecol or "event"
        ...
        if user_text:
            ...
            if low.startswith("train") and ("mysa" in low or "texgisa" in low):
                t = _get_arg(r"time(?:_col)?\s*=\s*([\w\.\-]+)", t_guess)
                e = _get_arg(r"event(?:_col)?\s*=\s*([\w\.\-]+)", e_guess)
    ...
    with right:
        ...
        if has_data:
            ...
            with st.form("direct_run_form", clear_on_submit=False):
                algo = st.selectbox("Algorithm", ["TEXGISA (MySA)", "CoxTime", "DeepSurv", "DeepHit"])
                ...
                lambda_expert = st.number_input("λ_expert (MySA)", 0.0, 5.0, 0.10, step=0.05)
